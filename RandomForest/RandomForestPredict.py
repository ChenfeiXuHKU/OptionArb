# -*- coding: utf-8 -*-
"""
Created on Thu Jan  3 13:50:18 2019
...
def SearchPortfolio(begin, name, dim, number):
    ...
    train_data = pd.read_csv('../train_data/train_data_' + str(dim) + 'd_' + name + '.csv')
    ...
    for i in range(data.shape[1]):
        prob = rf.predict_proba(data.iloc[:,i].values.reshape(1,dim*2))
        Predict_data.update({data.columns[i]:prob[0,1]})      
